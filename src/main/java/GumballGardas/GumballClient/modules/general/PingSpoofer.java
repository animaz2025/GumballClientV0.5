...
public class PingSpoofer extends Module {
    ...
    public PingSpoofer() {
        super(GumballClient.CATEGORY, "ping-spoofer", "Spoofs your ping to a specified value.");
    }
            ...
            for (int i = 0; i < entries.size(); i++) {
                ...
                if (mc.player.getUuid().equals(entry.profileId())) {
                    ...
                    try {
                        ...
                        GumballClient.Log("Error modifying ping: " + e.getMessage());
                    }
                }
            }
            
            if (modified) {
                GumballClient.Log("Successfully spoofed ping to " + fakePing.get() + "ms");
            }
        } catch (Exception e) {
            GumballClient.Log("Error in PingSpoofer: " + e.getMessage());
        }
    }
